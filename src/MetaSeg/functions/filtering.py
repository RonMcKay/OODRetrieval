...
class QuantileFilter(object):
    def __init__(self, quantile, lower=False, values=None):
        ...
        Bool values are calculated based on the specified quantile. If 'values' is not None, the quantile is calculated
        based on these. Otherwise the quantile is calculated on the fly in the call method.

        Args:
            quantile (float): Quantile value to use for filtering the ious. Should be in the interval (0,1)
            lower (bool): If True takes the quantile from the lower end of the distribution. Else from the upper end.
                Default: False
...
class PredFilter(object):
    def __init__(self, valid_classes):
        ...
        Args:
            valid_classes (sequence of ints): Sequence of classes that are used for filtering.
        """
