...
from .in_out import metrics_load, metrics_dump, get_indices
...
def concatenate_metrics(num_imgs, metrics_dir, save=False):
    ...
    if isinstance(num_imgs, int):
        ...
        raise ValueError('num_imgs should either be of type int, list or tuple but received {}'.format(type(num_imgs)))

    start = list([0, len(metrics["S"])])
    for i, k in enumerate(tqdm.tqdm(num_imgs, total=len(num_imgs) + 1, initial=1)):
        # sys.stdout.write("\t concatenated file number {} / {}\r".format(i + 1, len(num_imgs)))
        m = metrics_load(k, metrics_dir=metrics_dir)
        start += [start[-1] + len(m["S"])]
        for j in metrics:
            metrics[j] += m[j]
            # print("\nconnected components: {}".format(len(metrics['iou'])))
            # print("non-empty connected components: {}".format(np.sum(np.asarray(metrics['S_in']) != 0)))
        if save:
...
def metrics_to_nparray(metrics, names, normalize=False, non_empty=False, all_metrics=(), **kwargs):
...
def metrics_to_dataset(metrics, nclasses, non_empty=True, all_metrics=(), class_dtype=CONFIG.CLASS_DTYPE, **kwargs):
    ...
    if class_dtype == 'probs':
        ...
        raise ValueError(
            'class_dtype should be one of [\'one_hot_classes\', \'probs\'] but got {}.'.format(class_dtype))
...
def load_data(dataset='cityscapes',
              num_imgs=None,
    ...
    metrics, start = concatenate_metrics(num_imgs,
                                         save=False,
                                         metrics_dir=metrics_dir)
    nclasses = np.max(metrics['class']) + 1

    xa, classes, ya, _, x_names, class_names, \
        xa_mean, xa_std, classes_mean, classes_std = metrics_to_dataset(metrics,
                                                                        nclasses,
                                                                        non_empty=False,
                                                                        **kwargs)
    xa = np.concatenate((xa, classes), axis=-1)

    return xa, ya, x_names, class_names, xa_mean, xa_std, classes_mean, classes_std, start, metrics['class']
