...
def trainidtocolor(target):
    ...
    cols = np.array([trainid_to_color[c] for c in target.flatten()]).reshape((target.shape[0], -1))
...
def fulltocolor(target):
    ...
    rgb_target = np.concatenate([np.expand_dims(colors[t].reshape(3, h, w), 0) for t in target])
...
class CityscapesLAF(Dataset):
    def __init__(self, split='train',
        ...
        if self.split not in ['train', 'test']:
            raise ValueError('\'split\' should be one of [train, test] but received {}!'.format(self.split))
