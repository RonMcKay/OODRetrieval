...
def main(args, _run, _log):
    ...
    if args['load_file'] is None:
        ...
        inputs = [(ind,
                   iou_pred[start[i]:start[i + 1]],
                   thresholds) for i, ind in enumerate(inds)]
        ...
        with open(join(args['save_dir'], 'confusion_matrices_{}.p'.format(args['dataset'])), 'wb') as f:
    ...
    ax.plot(thresholds[thresholds < args['max_t']], np.array(ious_neg),
            color=colmap(4),
            linestyle='-')
    ax2.plot(thresholds[thresholds < args['max_t']], np.array(ious_pos),
             color=colmap(8),
             linestyle='-')

    plt.savefig(join(args['plot_dir'], 'iou_plot_{}.png'.format(args['dataset'])),
                dpi=args['dpi'],
                bbox_inches='tight')
    _log.info('Saved plot to \'{}\''.format(join(args['plot_dir'], 'iou_plot_{}.png'.format(args['dataset']))))
