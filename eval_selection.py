...
from src.MetaSeg.functions.in_out import probs_gt_load, get_indices
from src.datasets.a2d2 import trainid_to_name, id_to_trainid
...
def return_and_update_instances(components_gt, box):
    found_instances, instance_size = np.unique(components_gt[box[1]:box[3], box[0]:box[2]], return_counts=True)
    rel_instance_size = instance_size / np.unique(components_gt[np.isin(components_gt, found_instances)],
...
def config():
    args = dict(
        ...
        save_file_filtered=join(CONFIG.metaseg_io_path, 'filtering_count_eval_128_128_a2d2.p'),
...
def main(args, _run, _log):
            ...
            for cl in np.unique(gt):
                ...
                if args['file_total_count'] is None:
                    for c in range(1, counts_gt + 1):
                        ...
                        if (bottom - top) < args['min_height'] or (right - left) < args['min_width']:
                            continue
                        else:
                ...
                if ind in image_indices:
                    for b in boxes[(gt_segments == cl)
                                   & (image_level_index == np.argwhere(image_indices == ind).squeeze()), :]:
                        components_gt, instance_counts = return_and_update_instances(components_gt, b)
    ...
    for i, k in enumerate([key for key, v in num_instances.items() if v > 0] if sel_classes is None else sel_classes):
        if num_instances[k] > 0:
            ax = fig.add_subplot(n if sel_classes is None else 2,
                                 n if sel_classes is None else 3,
                                 i + 1)
            ax.text(
                0.5,
                1.0,
                '{}'.format(trainid_to_name[k] if not trainid_to_name[k][-1].isdigit() else trainid_to_name[k][:-2]),
            ...
            ax.pie(
                ...
                autopct=lambda pct: label_autopct(pct, [num_instances[k] - f_num_instances[k], f_num_instances[k]]),
                pctdistance=0.65,
                wedgeprops=dict(
                    width=1.0,
                    edgecolor='w',
    ...
    plt.savefig(
        join(args['plot_dir'],
             'instance_counts{}.{}'.format('' if sel_classes is None else '_selected', args['plot_filetype'])),
        dpi=args['dpi'],
    )
