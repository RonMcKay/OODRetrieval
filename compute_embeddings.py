...
from src.embedding_networks import feature_vgg16, feature_resnet18, feature_resnet101, feature_resnet152, \
    feature_densenet201, feature_wide_resnet101
...
from src.MetaSeg.functions.in_out import components_load, probs_gt_load, get_indices
...
# this mean and stardard deviation have been used for all PyTorch models during training. Use them during prediction
...
def get_image_index_to_components(component_indices,
                                  start):
    """Maps global component indices and start values to their local component indices and image index.
...
def get_embedding(image, net, args):
    """Computes the output of the supplied neural network with respect to the supplied image.
...
def get_component_pred(pred, segment_indices):
    """Computes the prediction of a segment based on the supplied predictions and segment indices."""
...
def config():
    args = dict(
        ...
        load_file=None,  # File in which segments got already extracted. If specified the file get's loaded and
                         # the embeddings in there are overwritten.
        gpu=CONFIG.GPU_ID,  # GPU id to use for computation of features for the embedding space
        n_jobs=CONFIG.NUM_CORES,  # Number of processes to use for the extraction of all bounding boxes
        min_height=128,  # Minimum height of a predicted segment
        min_width=128,  # Minimum width of a predicted segment
        min_crop_height=128,  # Minimum height of the resulting bounding box, can be larger than min_height
    ...
    args['save_file'] = join(CONFIG.metaseg_io_path, 'embeddings_{}_{}_{}.p'.format(args['min_height'],
                                                                                    args['min_width'],
                                                                                    args['net']))
...
def main(args, _run, _log):
    ...
    if args['load_file'] is None:
        ...
        for i, d in enumerate(args['datasets'][1:], start=1):
            _log.info('{} ...'.format(d))
            num_imgs = get_indices(join(CONFIG.metaseg_io_path,
                                        'metrics',
                                        'deeplabv3plus',
                                        d))
            xa_tmp, *_, start_tmp, pred_tmp = load_data(d,
                                                        num_imgs=num_imgs,
                                                        xa_mean=xa_mean,
                                                        xa_std=xa_std,
        ...
        if args['meta_model'] == 'neural':
            ya_pred_test = meta_nn_predict(pretrained_model_path=args['meta_nn_path'],
                                           x_test=xa_all,
                                           gpu=args['gpu'])
        elif args['meta_model'] == 'linear':
            ya_pred_test, _ = regression_fit_and_predict(x_train=xa, y_train=ya, x_test=xa_all)
        else:
        ...
        _log.info('\t{:^{width}s} | Filtered | Total'.format(
            'Class name',
            width=max([len(v[0]) for v in train_dat.pred_mapping.values()] + [len('Class name')])))
        for cl in np.unique(pred_test).flatten():
            _log.info('\t{:^{width}s} | {:>8d} | {:<8d}'.format(train_dat.pred_mapping[cl][0],
        ...
        for c in tqdm.tqdm(r):
            # load image
            preds, gt, image_path = probs_gt_load(c['image_index'], input_dir=join(CONFIG.metaseg_io_path,
                                                                                   'input',
                                                                                   c['model_name'],
                                                                                   c['dataset']),
                                                  preds=True)
        ...
        crops['embeddings'] = []
        for i, image_path in tqdm.tqdm(enumerate(crops['image_path']), total=len(crops['image_path'])):
            image = Image.open(image_path).convert('RGB')
